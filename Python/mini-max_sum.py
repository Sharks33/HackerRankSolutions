...
EXPLANATION
Our initial numbers are 1, 2, 3, 4, and 5. We can calculate the following sums using
...
As you can see, the minimal sum is 1 + 2 + 3 + 4 = 10 and the
maximal sum is 2 + 3 + 4 + 5 = 14. Thus, we print these minimal
...
min_val = min(long(i) for i in arr)
max_val = max(long(i) for i in arr)
min_sum = sum(arr) - max_val
max_sum = sum(arr) - min_val

print long(min_sum), long(max_sum)
